Feature: Sandbox Profile Management
  ...
  Scenario: Create a sandbox profile with custom parameters
    When I run "aitool sandbox create minimal --cpus 1 --memory 1024"
    Then the exit code is 0
    And the output contains "Created sandbox profile 'minimal'"
    When I run "aitool sandbox list"
  ...
  Scenario: Set active profile
    Given I create a sandbox profile "gpu" with cpus=5 and memory=8192
    When I run "aitool sandbox active gpu"
    Then the exit code is 0
  ...
  Scenario: Clear active profile with empty string
    Given I create a sandbox profile "gpu" with cpus=5 and memory=8192
    Given I run "aitool sandbox active gpu"
    When I run "aitool sandbox active"
    Then the exit code is 0
  ...
  Scenario: Delete profile
    ...
    When I run "aitool sandbox delete temp"
    Then the exit code is 0
  ...
  Scenario: Create a profile with volume mounts
    When I run "aitool sandbox create myprofile --cpus 2 --volume /data:ro --volume /logs"
    Then the exit code is 0
    And the output contains "Created sandbox profile 'myprofile'"
    When I run "aitool sandbox show myprofile"
    Then the exit code is 0
  ...
  Scenario: Create a profile deduplicates exact duplicate volumes
    When I run "aitool sandbox create deduped --cpus 1 --volume /data --volume /data"
    Then the exit code is 0
    When I run "aitool sandbox show deduped"
    Then the exit code is 0
